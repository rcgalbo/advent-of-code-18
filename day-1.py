...
part 1
calculate the frequency of the device
...
create an iterator for the data object
build a dictionary of the frequencies
return the value for the failure
'''
...
class Sequence:
    def __init__(self, data):
        self.data = data
        self.iter = iter(data)
        self.frequencies = list( accumulate(self.data) )
...
def search_duplicates(data):
    ...
    while len(frequencies) == len(set(frequencies)):
        new = [frequencies[-1]] + data
        frequencies.pop( -1 )
        frequencies += list(accumulate(new))
    freq = set()
...
# assert search_duplicates([+1, -1]) == 0
