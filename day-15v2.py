...
# replicating @fogelman solution
# no longer treating empty space as a commodity
# using sets
...
class Model:
    def __init__(self, cave_str, elf_attack=None):
        self.elf_attack = elf_attack
        self.walls = set()
        self.units = []
        self.rounds = 0

        for y, line in enumerate(cave_str.split('\n')):
    ...
    def get_attack(self, unit):
        units = [(x.hp, x.position, x) for x in self.units
            if x.team != unit.team and x.hp > 0 and
                distance(unit.position, x.position) == 1]
...
#.....#
#######"""
rounds, hp = Model(ex4).run()
assert rounds * hp == 27730

# part 1
with open('data/data-day15.txt') as f:
    cave_str = f.read()
rounds, hp = Model(cave_str).run()
print(rounds * hp)

# part 2
for elf_attack in count(4):
    try:
        rounds, hp = Model(cave_str, elf_attack).run()
        print(rounds * hp)
        break
    except Exception:
        pass